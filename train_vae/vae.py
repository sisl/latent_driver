...
class VariationalAutoencoder():
    ...
    def _create_policy(self, args):
        # Get smaples from standard normal distribution, transform to match z-distribution
        samples = tf.random_normal([args.sample_size, args.batch_size, args.z_dim], name="samples")
    ...
    def _create_optimizer(self, args):
        ...
        el1 = -0.5 * tf.to_float(args.z_dim)
        el2 = -tf.reduce_sum(tf.log(std_z), 1)
        el3 = tf.reduce_sum(tf.square(std_z), 1)
        el4 = tf.reduce_sum(tf.square(self.z_mean), 1)
        encoder_loss = el1 + el2 + el3 + el4

        # Find overall loss
        self.policy_cost = tf.reduce_mean(policy_loss/args.seq_length)
        self.cost = tf.reduce_mean((policy_loss + self.kl_weight*encoder_loss)/args.seq_length)
        self.summary_policy = tf.scalar_summary("Policy loss", tf.reduce_mean(policy_loss)/args.seq_length)
        self.summary_encoder = tf.scalar_summary("Encoder loss", tf.reduce_mean(encoder_loss)/args.seq_length)
        self.summary_loss = tf.scalar_summary("Overall loss", self.cost)
